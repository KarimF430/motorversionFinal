...
export const models = pgTable('models', {
  ...
  engineSummaries: jsonb('engine_summaries')
    .$type<
      {
        title: string;
        summary: string;
    ...
    >()
    .default([]),
  mileageData: jsonb('mileage_data')
    .$type<
      {
        engineName: string;
  ...
  keyFeatureImages: jsonb('key_feature_images')
    .$type<{ url: string; caption: string }[]>()
    .default([]),
  spaceComfortImages: jsonb('space_comfort_images')
    .$type<{ url: string; caption: string }[]>()
    .default([]),
  storageConvenienceImages: jsonb('storage_convenience_images')
    .$type<{ url: string; caption: string }[]>()
    .default([]),
...
export const variants = pgTable('variants', {
  ...
  highlightImages: jsonb('highlight_images')
    .$type<{ url: string; caption: string }[]>()
    .default([]),
...
export const popularComparisons = pgTable('popular_comparisons', {
  id: text('id').primaryKey(),
  model1Id: text('model1_id')
    .notNull()
    .references(() => models.id, { onDelete: 'cascade' }),
  model2Id: text('model2_id')
    .notNull()
    .references(() => models.id, { onDelete: 'cascade' }),
