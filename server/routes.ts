...
function formatBrandSummary(
  ...
  for (const line of lines) {
    ...
    if (
      trimmedLine.includes('Start of operations in India:') ||
      ...
      trimmedLine.includes('car price starts at') ||
...
export function registerRoutes(app: Express, storage: IStorage, backupService?: BackupService) {
  ...
  app.patch('/api/models/:id', async (req, res) => {
    try {
      ...
      console.log(
        ...
        model.storageConvenienceImages?.length || 0,
        'images'
      );
  ...
  app.get('/api/frontend/models/:slug', async (req, res) => {
    try {
      ...
      const model = models.find((m) => m.name.toLowerCase().replace(/\s+/g, '-') === slug);
