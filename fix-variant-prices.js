...
async function fixPrices() {
  ...
  try {
    ...
    for (const range of budgetRanges) {
      console.log(`💰 Processing ${range.name} (₹${range.min/100000}L - ₹${range.max/100000}L)...`);
    ...
    console.log(`  ₹8L - ₹15L:   ${prices.filter(p => p > 800000 && p <= 1500000).length} variants`);
    console.log(`  ₹15L - ₹25L:  ${prices.filter(p => p > 1500000 && p <= 2500000).length} variants`);
    console.log(`  ₹25L - ₹50L:  ${prices.filter(p => p > 2500000 && p <= 5000000).length} variants`);
